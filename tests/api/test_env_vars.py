...
async def test_list_masks_secret_value(async_client):
    ...
    with patch("docupipe_manager.api.env_vars._require_access_async", new=AsyncMock(return_value={"role": "admin"})):
        with patch("docupipe_manager.deps.get_engine", return_value=_mock_engine(rows=rows)):
...
async def test_create_plain_var(async_client):
    ...
    pid = uuid.uuid4()
    with patch("docupipe_manager.api.env_vars._require_access_async", new=AsyncMock(return_value={"role": "admin"})):
        with patch("docupipe_manager.deps.get_engine", return_value=_mock_engine(fetchone_row=None)):
...
async def test_create_duplicate_key_conflict(async_client):
    ...
    with patch("docupipe_manager.api.env_vars._require_access_async", new=AsyncMock(return_value={"role": "admin"})):
        with patch("docupipe_manager.deps.get_engine", return_value=_mock_engine(fetchone_row=existing)):
            r = await async_client.post(f"/docupipe/api/projects/{pid}/env-vars",
...
async def test_update_secret_without_value_field_keeps_original(async_client):
    ...
    with patch("docupipe_manager.api.env_vars._require_access_async", new=AsyncMock(return_value={"role": "admin"})):
        with patch("docupipe_manager.deps.get_engine", return_value=_mock_engine(fetchone_row=current)):
